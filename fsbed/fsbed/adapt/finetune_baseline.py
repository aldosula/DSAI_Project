...
def two_step_finetune(
    ...
    # Sample negatives from QUERY REGION (after support) like DCASE baseline
    # This is key: we want negatives from the same region we'll be predicting on
    query_length = num_frames - support_end_frame
    
    if query_length > 0:
        # Sample 100 random frames from query region (like DCASE baseline)
        target_neg_count = 100  # DCASE uses 100 negatives
        query_indices = torch.arange(support_end_frame, num_frames, device=device)
        
        if len(query_indices) > target_neg_count:
            perm = torch.randperm(len(query_indices), device=device)
            neg_frames = query_indices[perm[:target_neg_count]]
        else:
            neg_frames = query_indices
    else:
        # Fallback: use inter-segment negatives if no query region
